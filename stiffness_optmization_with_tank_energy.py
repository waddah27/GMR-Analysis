...
class optimal_stiffness:
    ...
    def run(self, x: ndarray, F_d: ndarray):
        ...
        solution = self.problem.solve(verbose=True, solver=self.solver, use_indirect=True)
...
if __name__ == "__main__":
    ...
    F_d = np.array([0, 0, 0])
    K_min = np.array([200.0, 200.0, 200.0])
    k_max = np.array([2000.0, 2000.0, 2000.0])
    epsilon = 0.3
    initial_tank_energy = 0.3  # Example initial energy
    trj = np.linspace(0,1,10)
    F_d_demo = np.array([[0, 0, i*600] for i in trj])
    x_tilde_demo = np.array([[i, i, i] for i in trj])
    kd_opt_arr = []
    ...
    for x_tilde, F_d in zip(x_tilde_demo, F_d_demo):
        try:
            k_d_opt, opt_solution, status, tank_energy = stiffness_optimizer.run(x_tilde, F_d)
            kd_opt_arr.append(k_d_opt)
        except Exception as E:
            print(f"Failed at {i} -- Reason: {E}")
